...
import {
	...
	getTransactions,
} from "@/app/(application)/dashboard/[id]/transactions/_actions";
...
export default function TransactionsPage() {
	...
	}, [type, selectedMonth, selectedYear, customDateStart, customDateEnd, currentDate.getFullYear, currentDate.getMonth]);
	...
	const { data: transactionsDate = [], isLoading } = useQuery({
		...
		queryFn: async () => await getTransactions(queryParams),
		retry: 1,
		refetchOnMount: true,
	...
	const groupedTransactions = useMemo(() => {
		...
		filteredTransactions?.forEach((transaction) => {
			const dateObj = parseISO(transaction.date.toString());
	...
	return (
		<div className="space-y-5 pb-24">
			<SectionTemplate className="pt-4">
				...
				{type === "month" && <MonthYearSelector className="mb-6" />}
